  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        // 👇 fix partial Cloudinary path
        if (data.architecture_diagram) {
          data.architecture_diagram = data.architecture_diagram.startsWith("http")
            ? data.architecture_diagram
            : `${CLOUDINARY_BASE}${data.architecture_diagram.replace(
                "127.0.0.1",
                "localhost"
              )}`;
        }
